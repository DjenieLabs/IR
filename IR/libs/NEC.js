...
* Date: 08/Jul/2017
* Last Modified: 17/10/2017, 12:11:43 pm
...
*
*      Repeat codes: 9ms burst + 2.25ms space + 562.5 end of message every 40ms
...
define(['helpers'], function(h){
	...
	function NEC(){
		this.protocol = {
			...
			repeat:{burst: 9000, space: 2250, end: 562.5, timeout: 108 /* Timeout between repeats in ms*/  }
		};
	...
	function decode(raw){
		if(h.inRange(raw[0], 9000)){
			if(h.inRange(raw[1], 4500)){
				...
				return res;
			}else if(h.inRange(raw[1], this.protocol.repeat.space)){
				if(this._lastCode !== undefined){
					if((raw.length == 3) && h.inRange(raw[2], this.protocol.repeat.end)){
						// Now check if the previous code was within the repeat spaces
						var elapsed = h.millis() - this._lastAt;
						if(elapsed < this.protocol.repeat.timeout || h.inRange(elapsed, this.protocol.repeat.timeout)){
							this._lastAt = h.millis();
